...
function filterObjectsByKey(arr, key) {
  ...
  const a = arr.filter((arr) => arr.hasOwnProperty(key));
  return a;
...
function checkAndPreventExtensions(obj) {
  ...
  const a = Object.isExtensible(obj);
  if (a === true) {
    Object.freeze(obj);
  }
  const b = Object.isExtensible(obj);
  return b;
...
function checkOwnership(obj1, obj2, key) {
  const a = obj1.hasOwnProperty(key);
  const b = obj2.hasOwnProperty(key);
  if (a === key && b === key) {
    return a === b;
  }
  if (a === b) {
